...
- **Single-render in user-facing summaries.** When rendering a list of PA items in any user-facing surface (morning brief, scan-and-check output, ad-hoc query response), each PA item appears in exactly one section per render. Apply this precedence: (1) synthesized top-of-mind list, (2) today's calendar inline context, (3) attention/urgent buckets, (4) awaiting others, (5) other lenses (e.g. week-ahead preview). Lower-precedence sections may use a `(see Top 3 #N)` or `(see PA-N)` pointer stub but must never re-render the full item line. Rationale: the morning brief used to triple-render hot items (e.g., PA-186 capital call appearing in Week Ahead Preview + URGENT + Open Tasks on the same brief), making briefs overwhelming and obscuring what actually mattered. Authoritative implementation: `prompts/morning-brief.md` Step 8.5.
