...
| source = calendar AND source_ref starts with `conflict:` | Re-check the event pair across the **conflict-eligibility calendar set** (CLAUDE.md "Calendar set" → `primary` + `personal`; family/holidays are not in scope here since the morning-brief sweep never creates conflict items from those). Emit 2 parallel `gcal_list_events` calls (timeMin = item due_date 00:00 ET, timeMax = +1 day) — one for `primary`, one for `personal` — and merge results into a {event_id → event_obj} map. If either event ID in the source_ref is missing from the merged map, declined, marked transparent, or the pair no longer overlaps / is no longer a tight transition, set status = `done` and append to Notes: `Auto-resolved {YYYY-MM-DD}: conflict cleared`. Do NOT create new conflict items here — creation happens only in the morning brief sweep. |
